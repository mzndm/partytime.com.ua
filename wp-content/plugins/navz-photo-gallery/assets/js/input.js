(function( $ ){
		...
		if( $('#acf-photo-gallery-metabox-add-images').length > 0 ){
			if ( typeof wp !== 'undefined' && wp.media && wp.media.editor){
				$('.wrap').on('click', '#acf-photo-gallery-metabox-add-images', function(e){
					...
					if( $('#acf-photo-gallery-metabox-list li.acf-photo-gallery-media-box-placeholder').length > 0 ){
						$('#acf-photo-gallery-metabox-list li.acf-photo-gallery-media-box-placeholder').remove();
					}
				return false;
				});
			}
	...
	function acf_photo_gallery_remove_photo(){
		...
		$(document).on('click', '#acf-photo-gallery-metabox-list .dashicons-dismiss', function(){
			...
			if( confirm('You are about to remove this photo from the gallery. Are you sure?')){
				$.get(url, function( data ){
					...
					if( $('#acf-photo-gallery-metabox-list li').length < 1 ){
						$('#acf-photo-gallery-metabox-list').append('<li class="acf-photo-gallery-media-box-placeholder"><span class="dashicons dashicons-format-image"></span></li>');
					}
				});
			}
			return false;
		...
		$(document).on('click', '#acf-photo-gallery-metabox-edit .acf-edit-photo-gallery button.button-primary', function(){
			...
			target = $('#acf-photo-gallery-metabox-edit #acf-photo-gallery-metabox-edit-' + attachment + ' input[name=\'target\']').is(":checked");
			...
			data = {'field': field, 'post': post, 'attachment': attachment, 'action': action, 'nonce': nonce,	'url': url, 'target': target, 'title': title, 'caption': caption};
			button = $(this);
